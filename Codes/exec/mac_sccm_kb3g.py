...
OPT5 = [False,False,True]

#compile_hfb   = True #False
compile_hfb,compile_pnamp,compile_gcm =OPT5 

# parameters
run_hfb,run_pnamp,run_gcm = OPT4
...
if compile_hfb:
   ...
   copyfile("mac_makefile4HFB","makefile")
   os.system("make")
...
if compile_pnamp:
    ...
    for filename in os.listdir(dir_src):
        if filename.endswith('.f90'):
            shutil.copy(dir_src + filename, dir_dst)
    copyfile("mac_makefile4PNAMP","makefile")
    os.system("make")
...
if compile_gcm:
    ...
    for filename in os.listdir(dir_src):
        if filename.endswith('.f90'):
            shutil.copy(dir_src + filename, dir_dst)
    copyfile("mac_makefile4GCM","makefile")
    os.system("make")
