...
!+---------------------------------------------------------------------+
!    This definition is different from djmk (Edmonds) by a factor of (-1)^(M-MP)
!    or it is d^I_M, MP (-bet) of Edmonds
!+---------------------------------------------------------------------+
...
! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
      ...
      pre = 0.5*(fact(IplusM)+fact(IminusM)+fact(IplusMP)+fact(IminusMP))
