...
!     .............................................
      ...
      do J1=J2,min(J2+2,JJmax),1
         ...
         do K2 =-J2,J2,1
           do K1 =-J1,J1,1
              Kernel%qpred(J2,K1,K2,J1-J2)  = zzero
              Kernel%qcpred(J2,K1,K2,J1-J2) = zzero
           enddo ! K1
         enddo ! K2
...
!     .............................................
                   ...
                   do K1 =-J2,J2,1
                    ! Wigner D-function
                       ...
                       zbp=zone*dwignerI_gen('i',Ji,K1,K2,bet)
                       zbm=zone*dwignerI_gen('i',Ji,K1,-K2,bet)
                       zgp=cdexp(Zimag*K2*gam)
                       zgm=cdexp(-Zimag*K2*gam)

                      za1 =              zap*zgp*zbp   
                      za2 = iv(Ji+K1+K2)*zap*zgm*zbm  
                      za3 = iv(K1+K2)   *zam*zgm*zbp  
                      za4 = iv(Ji)      *zam*zgp*zbm  

                   do io=1,8
                      zo1 = ZRES(iang1,io)   
                      ZsumJKK(io,K1,K2)=ZsumJKK(io,K1,K2) &
                        + weightJ(Ji)*(1+iv(abs(K1)))*(1+iv(abs(K2))) &
              ...
              !  transfer from Q_lm to M_lm
              !  zmlm(-2,2) = (zqlm(1,-2,2)+zqlm(1,2,2))/sqrt(2.0)  
              !  zmlm(-1,2) = (zqlm(1,-1,2)-zqlm(1,1,2))/sqrt(2.0)  
              !  zmlm(0,2)  =  zqlm(1,0,2)  
              !  zmlm(1,2)  = (zqlm(1,1,2)+zqlm(1,-1,2))/sqrt(2.0)  
              !  zmlm(2,2)  = (zqlm(1,2,2)-zqlm(1,-2,2))/sqrt(2.0)  
              !  reduced E2 matrix elements:  <J1,K1,q1 ||e Q2|| J2,K2,q2> 
              ! **************************
               ...
               do lmu=-2,2  !mu' 
                   ...
                   za1 = (zap*zgp*zbp + iv(abs(Ji+K2p+K2+lmu))*zap*zgm*zbm)
                   za2 = (iv(abs(2+K2p+K2))*zam*zgm*zbp &
                         +iv(abs(J2+2-lmu))*zam*zgp*zbm)

                   ! d^J_(K2p,K2) (-bet): dwignerI_gen('i',J,K2p,K2,bet)
                   zo1 = zqlm(1, lmu,2) !ZRES(iang1,io)
                   zo2 = zqlm(1,-lmu,2) !ZRES(iang1,io)
...
!                 ............. exchange q1 and q2
                 do nu=-2,2,1
                   ...
                   za1 = (zam*zgm*zbp*zbp2 +iv(abs(Ji+K2+2+K2p+lmu))*zam*zgp*zbm*zbm2)
                   za2 = (iv(abs(K2p+K2+nu+lmu-2))*zap*zgp*zbp*zbp2 &
                         +iv(abs(Ji+nu))*zap*zgm*zbm*zbm2)
