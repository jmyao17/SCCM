        subroutine Kernel4Crankingx(lpr,Nphi1,Nphi2,NLEV)
        USE VAPHFB_PAR
        ! .................................
...
!      ................ 1B,2B,densities in J-scheme
        ...
        complex*16 zqlm(1:4,-2:2,2),zqlm2(1:4,-2:2,2)

        real*8 t0,t1,t2,t3,t4,t5,t6,t7,t8 !,omp_get_wtime()

        t0 = omp_get_wtime()
        write(*,*) ' -> Start to compute kernels for cranking states ..'
!  ....................... preparation
...
!  ..........................
        do J=0,JJmax
           ...
           weightJ(J) = (2.d0*J+1.d0)/(2.d0*2)  ! trapezoidal integration rule for alpha (range devided by 2) and gamma  
        enddo
      !read wave functions
...
!       write(121,'(a,i4,a,3f8.3,a,17f12.5)')  '<phi1|R(',iang,':', alp, bet, gam, ')|phi2>=', &
            ...
            DO ibet=1,max(1,NLEG_BET_Max) !start do loop in beta
              ...
              DO igam=1,AMP%NLEG_GAM !start do loop in gamma
                 ...
                 stop 'ieuler_ang is not given properly'
                   ...
                   ! <O R(a,b,g)>
                   iang1 = ieuler_ang(ialp,ibet,igam)
                   ! <O R(a,b,2pi-g)>
                   iang2 = ieuler_ang(ialp,ibet,AMP%NLEG_GAM+1-igam)
                   ! <O R(pi-a,b,g)>
                   iang3 = ieuler_ang(AMP%NLEG_ALP+1-ialp,ibet,igam)
                   ! <O R(pi-a,b,2pi-g)>
                   iang4 = ieuler_ang(AMP%NLEG_ALP+1-ialp,ibet,AMP%NLEG_GAM+1-igam) 
                ...
                do K1 =-J2,J2,1
                 ! Wigner D-function
                   ...
                   zbm=zone*dwignerI_gen('i',J2,K1,K2,pi-bet)
                 ...
                 !  d0k2 = 1.d0   
                   ...
                   do io=1,8
                      zo1 = ZRES(iang1,io)   
                      zo2 = dconjg(ZRES(iang2,io))   
                      zo3 = dconjg(ZRES(iang3,io))   
                      zo4 = ZRES(iang4,io)   
                      ZsumJKK(io,K1,K2)=ZsumJKK(io,K1,K2) &
                        +weightJ(J2)*(za1*zo1+za2*zo2+za3*zo3+za4*zo4)*zwdvol 
                   end do
                ...
                zqlm(1,-2,2) = ZRES(iang1,9) 
                zqlm(2,-2,2) = ZRES(iang2,9) 
                zqlm(3,-2,2) = ZRES(iang3,9) 
                zqlm(4,-2,2) = ZRES(iang4,9)

                zqlm(1,-1,2) = ZRES(iang1,10) 
                zqlm(2,-1,2) = ZRES(iang2,10)
                zqlm(3,-1,2) = ZRES(iang3,10)
                zqlm(4,-1,2) = ZRES(iang4,10)

                zqlm(1,0,2) = ZRES(iang1,11)
                zqlm(2,0,2) = ZRES(iang2,11)
                zqlm(3,0,2) = ZRES(iang3,11)
                zqlm(4,0,2) = ZRES(iang4,11)

                zqlm(1,1,2) = ZRES(iang1,12)
                zqlm(2,1,2) = ZRES(iang2,12)
                zqlm(3,1,2) = ZRES(iang3,12)
                zqlm(4,1,2) = ZRES(iang4,12)

                zqlm(1,2,2) = ZRES(iang1,13)
                zqlm(2,2,2) = ZRES(iang2,13)
                zqlm(3,2,2) = ZRES(iang3,13)
                zqlm(4,2,2) = ZRES(iang4,13) 
!              ........... conjugate
                zqlm2(1,-2,2) = dconjg(ZRES(iang1,14)) 
                zqlm2(2,-2,2) = dconjg(ZRES(iang2,14))
                zqlm2(3,-2,2) = dconjg(ZRES(iang3,14))
                zqlm2(4,-2,2) = dconjg(ZRES(iang4,14))

                zqlm2(1,-1,2) = dconjg(ZRES(iang1,15))
                zqlm2(2,-1,2) = dconjg(ZRES(iang2,15))
                zqlm2(3,-1,2) = dconjg(ZRES(iang3,15))
                zqlm2(4,-1,2) = dconjg(ZRES(iang4,15))

                zqlm2(1,0,2) = dconjg(ZRES(iang1,16))
                zqlm2(2,0,2) = dconjg(ZRES(iang2,16))
                zqlm2(3,0,2) = dconjg(ZRES(iang3,16))
                zqlm2(4,0,2) = dconjg(ZRES(iang4,16))
!
                zqlm2(1,1,2) = dconjg(ZRES(iang1,17))
                zqlm2(2,1,2) = dconjg(ZRES(iang2,17))
                zqlm2(3,1,2) = dconjg(ZRES(iang3,17))
                zqlm2(4,1,2) = dconjg(ZRES(iang4,17))
!                
                zqlm2(1,2,2) = dconjg(ZRES(iang1,18))
                zqlm2(2,2,2) = dconjg(ZRES(iang2,18))
                zqlm2(3,2,2) = dconjg(ZRES(iang3,18))
                zqlm2(4,2,2) = dconjg(ZRES(iang4,18))
...
!
!                   Kernel%qpred(J2,K1,K2,0) = Kernel%qpred(J2,K1,K2,0) &
!     &              + weightJ(J2)*(2*J1+1)*iv(J1-K1)             &
!     &              * wignei(J1,2,J2,-K1,lmu,K2p)                &
!     &              *(za1*zqlm(1,lmu,2)+za2*zqlm(2,lmu,2)        &
!     &               +za3*zqlm(3,lmu,2)+za4*zqlm(4,lmu,2))*zwdvol
!               enddo !mlmu 
!             enddo ! k1           
!            enddo ! k2           
...
!                  spectroscopic quadrupole moment
     ...
     &              + weightJ(J2)*(2*J1+1)*iv(J1-K1)             &
     &              * wignei(J1,2,J2,-K1,lmu,K2p)                &
     &              *(za1*zqlm(1,lmu,2)+za2*dconjg(zqlm(2,lmu,2))&
     &               +za3*dconjg(zqlm(3,lmu,2))+za4*zqlm(4,lmu,2))*zwdvol
            ...
            do J1=J2,min(J2+2,JJmax)
              ...
              do mu=-2,2
                ...
                ! if(abs(K2p).gt.J2) cycle
                   ...
                   zbm=zone*dwignerI_gen('i',J2,K2,K2p,pi-bet)
                   zbm2=zone*dwignerI_gen('i',2,nu,mu,pi-bet)

                   za1 = zam*zgm*zbp*zbp2
                   za2 = zap*zgm*zbp*zbp2
                   za3 = zam*zgm*zbm*zbm2
                   za4 = zap*zgm*zbm*zbm2

                      Kernel%qcpred(J2,K1,K2,J1-J2)         &
     &             =  Kernel%qcpred(J2,K1,K2,J1-J2)         &
     &              + weightJ(J2)*(2*J1+1.0)*iv(abs(J1-K1)) &
     &              * wignei(J1,2,J2,-K1,mu,K2p)            &
     &              *(za1*zqlm2(1,nu,2)+za2*dconjg(zqlm2(2,nu,2))   &
     &               +za3*dconjg(zqlm2(3,nu,2))+za4*zqlm2(4,nu,2))*zwdvol
               enddo !nu 
               enddo !mu 
             enddo ! k1           
            enddo ! k2
...
!     &            dreal(Kernel%qpred(J2,K1,K2,2)/ZsumJKK(1,K1,K2))
         endif
        enddo
        enddo
!     ----------- 
