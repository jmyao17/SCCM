...
!      ................ 1B,2B,densities in J-scheme
        ...
        write(*,*) ' -> Start to compute kernels general...'
!  ....................... preparation
...
! .......................... loop over gamma angle
        DO kk=1,AMP%NLEG_GAM
           ...
           if(Input%icr.eq.2 .and. mod(iang,icheck).eq.0) call progress_bar(int(iang/icheck)) 
...
!      ................ integration over angles
        ...
        DO ialp=1,AMP%NLEG_ALP !start do loop in alpha
         alp=AMP%alpha(ialp)
         zwa=zone*AMP%wwalp(ialp)


        DO igam=1, AMP%NLEG_GAM !start do loop in gamma
         gam=AMP%gamma(igam)
         zwg=zone*AMP%wwgam(igam)
...
!         zgm=cdexp(-Zimag*Kp*gam)
        ...
        ! do K1 =-Ji,Ji,1
              ! Wigner D-function
               ...
               test = test + weightJ(Ji)*zap*zgp*zbp*zfac

                   do io=1,8
                      zo1 = ZRES(ieuler,io)
                      ZsumJKK(io,K1,K2)=ZsumJKK(io,K1,K2) &
                        +weightJ(Ji)*za1*zo1*zfac
                   end do
...
!      ............................. print out density matrix elements for g.s.
     ...
     &            dreal(Kernel%qcpred(J2,K1,K2,2)) !/ZsumJKK(1,K1,K2))
         endif
       ! enddo
       ! enddo
