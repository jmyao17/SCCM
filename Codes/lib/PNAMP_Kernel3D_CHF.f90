...
!  ..........................
        do J=0,JJmax
           weightJ(J) = 2.d0*(2.d0*J+1.d0)/(8.d0*pi**2)  
           if(AMP%NLEG_ALP*AMP%NLEG_GAM .eq. 1) weightJ(J) = (2.d0*J+1.d0)/(2.d0)  
        enddo
      !read wave functions
              ...
              DO igam=1,AMP%NLEG_GAM !start do loop in gamma
                ...
                do K1 =-J2,J2,1
                 ! Wigner D-function
                   ...
                   if(K1.eq.0) d0k1 = 2.d0  
                   if(K2.eq.0) d0k2 = 2.d0  
...
!              ........... conjugate
!                zqlm2(1,-2,2) = ZRES(iang1,14) 
!                zqlm2(2,-2,2) = ZRES(iang2,14)
!                zqlm2(3,-2,2) = ZRES(iang3,14)
!                zqlm2(4,-2,2) = ZRES(iang4,14)
!
!                zqlm2(1,-1,2) = ZRES(iang1,15)
!                zqlm2(2,-1,2) = ZRES(iang2,15)
!                zqlm2(3,-1,2) = ZRES(iang3,15)
!                zqlm2(4,-1,2) = ZRES(iang4,15)
!
!                zqlm2(1,0,2) = ZRES(iang1,16)
!                zqlm2(2,0,2) = ZRES(iang2,16)
!                zqlm2(3,0,2) = ZRES(iang3,16)
!                zqlm2(4,0,2) = ZRES(iang4,16)
!
!                zqlm2(1,1,2) = ZRES(iang1,17)
!                zqlm2(2,1,2) = ZRES(iang2,17)
!                zqlm2(3,1,2) = ZRES(iang3,17)
!                zqlm2(4,1,2) = ZRES(iang4,17) 
!                
!                zqlm2(1,2,2) = ZRES(iang1,18)
!                zqlm2(2,2,2) = ZRES(iang2,18)
!                zqlm2(3,2,2) = ZRES(iang3,18)
!                zqlm2(4,2,2) = ZRES(iang4,18)
...
!          <J1,K1,q2 ||e Q2|| J2,K2,q1> = <J2,K2,q1 ||e Q2|| J1,K1,q2>
!          since K1=K2, then simply exchange K1,K2 
              if(Nphi1.ne.Nphi2) then
                do K1 =-J1,J1,1
                do K2 =-J2,J2,1
                   Kernel%qcp(J2,K1,K2) = Kernel%qp(J2,K2,K1)      
                enddo ! k1           
                enddo ! k2           
              endif
!     .........................................................   
...
!     ......exchange q1 and q2 (Not K1 and K2): 
!     ...... <J1,K1,q2 ||e Q2||   J2,K2,q1> is calculated by <J2,K2,q1 ||e Q2||  J1,K1,q2>
!          <J1,K1,q2 ||e Q2||   J2,K2,q1> is saved as  Kernel%qcpred(J2,K1,K2) 
           if(Nphi1.ne.Nphi2) then
            ...
            do K2 =-J2,J2,1
             ...
             do K1 =-J1,J1,1
               do lmu=-2,2  !mu 
                   K1p = K2-lmu !miis = -lmu
                   if(abs(K1p).gt.J1) cycle
                   !zexp_alp = cdexp(Zimag*K2p*alp)


                   zap=cdexp(Zimag*K1p*alp)
                   zam=cdexp(-Zimag*K1p*alp)
                   zbp=zone*dwignerI_gen('i',J1,K1p,K1,bet)
                   zbm=zone*dwignerI_gen('i',J1,K1p,K1,pi-bet)
                   zgp=cdexp(Zimag*K1*gam)
                   zgm=cdexp(-Zimag*K1*gam)
...
!                  spectroscopic quadrupole moment
     ...
     &              + weightJ(J2)*(2*J1+1)*iv(J2-K2)             &
     &              * wignei(J2,2,J1,-K2,lmu,K1p)                &
     &              *(za1*zqlm(1,lmu,2)+za2*zqlm(2,lmu,2)        &
     &               +za3*zqlm(3,lmu,2)+za4*zqlm(4,lmu,2))*zwdvol
...
!      ............................. print out density matrix elements for g.s.
       ...
       do K2=-J2,J2 !ZsumJKK(io,K1,K2)
         ...
         if(abs(ZsumJKK(1,K1,K2)).gt. 1.d-6) then 
     ...
     &            dreal(Kernel%qp(J2,K1,K2)/ZsumJKK(1,K1,K2))
         ...
         if(abs(ZsumJKK(1,K1,K2)).gt. 1.d-6) & 
         hjkk(J2,K1,K2) =  dreal(ZsumJKK(7,K1,K2)/ZsumJKK(1,K1,K2))
        enddo
        enddo
!     ----------- 
   ...
   14 format(' J=',i3,'(',2i3,') N^J=',f10.4,      &
     &       ' H^J=',f10.4,' Z(N)^J=',2f6.2,    &
     &       ' J(J+1)=',f8.3,' Qp=',f8.3)
