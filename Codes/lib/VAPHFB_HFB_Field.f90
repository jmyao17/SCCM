...
! ................... spherical?
           ! if(tnljm%t(ia) .ne. tnljm%t(ic)     &
           !   .or. tnljm%lj(ia) .ne. tnljm%lj(ic) &
           !   .or. tnljm%twom(ia) .ne. tnljm%twom(ic) ) then 
           !    zham(ia,ic)  = 0.d0
           !    zGamma(ia,ic)= 0.d0
           ! endif
!  ..................w/o np-mixing 
...
!         .... (ab) <-> (cd)
            ...
            if(ic.ne.id) zDelta10(id,ic)=zDelta10(id,ic)-0.5*H%ME2BM(iabcd)*zkapa10(ia,ib)*2
            if(ic.ne.id) zDelta01(id,ic)=zDelta01(id,ic)-0.5*H%ME2BM(iabcd)*zkapa01(ia,ib)*2
       enddo ! iabcd
...
! ................... spherical?
          !  if(tnljm%t(ia) .ne. tnljm%t(ib)     &
          !    .or. tnljm%lj(ia) .ne. tnljm%lj(ib) &
          !    .or. abs(tnljm%twom(ia)) .ne. abs(tnljm%twom(ib)) ) then
          !   zDelta10(ia,ib) = 0.d0
          !   zDelta01(ia,ib) = 0.d0
          !  endif
...
!   ........ pairing collaps for only either neutron or proton
       if(HFB%ide(0)+HFB%ide(1).eq.1) then
       do it=0,1
          if(HFB%ide(it).ne.0) cycle
          do ia=it*NLEV/2+1,it*NLEV/2+NLEV/2
          do ib=it*NLEV/2+1,it*NLEV/2+NLEV/2
             zDelta10(ia,ib) = 0.001 
             zDelta10(ib,ia) =-0.001 
             zDelta01(ia,ib) =-0.001 
             zDelta01(ib,ia) = 0.001 
          enddo
          enddo
       enddo
       endif
