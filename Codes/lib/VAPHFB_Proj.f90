...
!       ...gradients
        ...
        if(NFOM .gt. 1) then
           ...
           zDetvac=Det !dsqrt((1./Det))*zone
           !print *, 'zDetvac=',zDetvac
         endif
...
!!!!!!! initializing gradients
      ...
      if(.NOT. ALLOCATED(Zham))      ALLOCATE(Zham(NLEV,NLEV))
       ...
       DO LLPN=1,NFOM**2   !Fomenko                
        ...
        !print *, 'Gauge angles(',Nphin(LLPN),Nphip(LLPN),')'
...
!    norm overlap
      ...
      ! write(*,*) '.... NZPROJ_VAP: Pfaffian ........'      
         ...
         znorm =snpfaf*zDetvac*ZdetPfaf*zDetvac*zphas_p*zphas_n
        !print *,'ZdetPfaf=',ZdetPfaf
        print *,'Znorm=',znorm
...
!    ..................... store the gamma   
        if(NFOM.eq.1 .or. LLPN.eq.1) then
...
!      commented by jmyao
      ...
      if(NFOM .gt. 1) then
        call zbareover(ZV0,ZU0,ZZc,Det,NLEV)
        zDetvac=Det*zone !dsqrt((1./Det))*zone
      endif 
