...
!      ........................ end PNP
     ...
     &        dreal(ZRES(iang,8)/ZRES(iang,1)),ZRES(iang,11),ZRES(iang,12),ZRES(iang,16),ZRES(iang,17)      
      endif
...
!          <J1,K1,q2 ||e Q2||   J2,K2,q1> is saved as  Kernel%qcpred(J2,K1,K2) 
      ...
      !&              + weightJ(iis)*(2*ifs+1)*iv(ifs)*iv(abs(mu-nu)) &
      !&              * dwignerI_gen('i',2,-nu,-mu,bet)                 &
      !&              * zqlmc(nu,2)   &
      !&              * dwignerI_gen('i',iis,0,miis,bet)                 &
      !&              * wignei(iis,2,ifs,miis,mu,0)                     &
      !&               * zfac 
      !        enddo ! nu
      !        enddo ! nu
